...
from dense_fusion.evaluate_likelihood import subRandomSigmaSearchEvery, evaluateYCBEvery
...
print('#'*100)
data = np.load('bingham_every.npz')
#likelihoods = data['likelihoods']
sigma = data['sigma']
print('Using sigma {}'.format(sigma))
eval_func = partial(evaluateYCBEvery, df_estimator, sigma=sigma)
likelihoods = evaluateYCBDataset(eval_func, dataset_root, test_filenames)
np.savez('bing_every_{}.npz'.format(sigma), likelihoods = likelihoods, sigma = sigma)
