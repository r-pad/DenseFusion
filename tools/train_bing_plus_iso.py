...
from lib.network import PoseNetPlusIsoBing
...
from object_pose_utils.utils.bingham import makeBinghamM
...
def main():
    ...
    opt.outf = 'trained_models/ycb_plus_iso_bing' #folder to save trained models
    opt.log_dir = 'experiments/logs/ycb_plus_iso_bing' #folder to save logs
    opt.repeat_epoch = 1 #number of repeat times for one epoch training
    estimator = PoseNetPlusIsoBing(num_points = opt.num_points, num_obj = opt.num_objects)
    estimator.cuda()
    ...
    criterion_lik = IsoLoss(opt.num_points_mesh, opt.sym_list)
        ...
        for rep in range(opt.repeat_epoch):
            for i, data in enumerate(dataloader, 0):
                ...
                points, choose, img, quat, target, model_points, idx = Variable(points).cuda(), \
                                                                 ...
                                                                 Variable(idx).cuda()
                pred_r, pred_t, pred_c, pred_s, emb = estimator(img, points, choose, idx)
                ...
                max_s = pred_s[0,which_max.item()]

                loss_lik, lik = criterion_lik(max_q.view(-1), 
                        torch.abs(max_s.view(-1)), quat)
                ...
                if train_count % opt.batch_size == 0:
                    ...
                    #logger.info('{}'.format(max_s.view(-1)))
                    optimizer.step()
                    optimizer.zero_grad()
                    train_dis_avg = 0
                    train_lik_avg = 0
