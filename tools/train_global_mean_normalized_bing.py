...
from lib.loss_bing import Loss_refine
from lib.utils import setup_logger
from object_pose_utils.datasets.pose_dataset import OutputTypes as otypes
from object_pose_utils.datasets.ycb_dataset import YcbDataset as YCBDataset
from object_pose_utils.datasets.image_processing import ColorJitter, ImageNormalizer
from object_pose_utils.datasets.point_processing import PointShifter, PointMeanNormalizer
...
def main():
    ...
    opt.outf = 'trained_models/ycb_mnorm' #folder to save trained models
    opt.log_dir = 'experiments/logs/ycb_mnorm' #folder to save logs
    ...
    for epoch in range(opt.start_epoch, opt.nepoch):
        ...
        for rep in range(opt.repeat_epoch):
            for i, data in enumerate(dataloader, 0):
                points, choose, img, true_r, true_t, idx = data
                idx = idx - 1
                points, choose, img, true_r, true_t, idx = Variable(points).cuda(), \
                                                                 Variable(choose).cuda(), \
                                                                 Variable(img).cuda(), \
                                                                 Variable(true_r).cuda(), \
                                                                 Variable(true_t).cuda(), \
                                                                 Variable(idx).cuda()
                pred_r, pred_t, pred_c, emb = estimator(img, points, choose, idx)
                loss, dis, new_points, new_target = criterion(pred_r, pred_t, pred_c, true_r, true_t, idx, points, opt.w, opt.refine_start)
        ...
        for j, data in enumerate(testdataloader, 0):
            ...
            points, choose, img, true_r, true_t, idx = Variable(points).cuda(), \
                                                             ...
                                                             Variable(idx).cuda()
            pred_r, pred_t, pred_c, emb = estimator(img, points, choose, idx)
            _, dis, new_points, new_target = criterion(pred_r, pred_t, pred_c, true_r, true_t, idx, points, opt.w, opt.refine_start)
