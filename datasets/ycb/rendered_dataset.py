...
class RenderedPoseDataset(data.Dataset):
    def __init__(self, num_pt, add_noise, root, noise_trans, refine):
        self.path = 'datasets/ycb/dataset_config/rendered_data_list.txt'
    ...
    def __getitem__(self, index):
        ...
        quat = np.load('{0}/{1}-trans.npy'.format(self.root, self.list[index]))
        ...
        mask = np.array(img.split()[-1])==255
        mask_back = ma.getmaskarray(ma.masked_equal(mask, 0))
